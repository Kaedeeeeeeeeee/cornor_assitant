...
- Upload App Store screenshots from `screenshots/store/<locale>`.
...
```bash
PATH="/opt/homebrew/opt/ruby/bin:$PATH" ASC_APP_VERSION=1.0.1 bundle exec fastlane mac submit_for_review build_number:5
```
...
```text
PEEK_BUILD_CERTIFICATE_BASE64      # base64 of a .p12 containing signing identities; generated by the helper script
PEEK_P12_PASSWORD                  # password for the .p12
...
Alternatively, instead of `PEEK_BUILD_CERTIFICATE_BASE64`, you can manually
provide both `PEEK_APP_CERTIFICATE_BASE64` and
`PEEK_INSTALLER_CERTIFICATE_BASE64` as separate `.p12` files.
...
By default, `script/prepare_fastlane_screenshots.sh` copies the localized screenshot set from `screenshots/store/<locale>` to `fastlane/screenshots/<locale>`.
...
Regenerate the source screenshots with:

```bash
python3 script/create_store_screenshots.py
```

## Source of Truth

- Metadata source: `CornerAssistantApp/docs/AppStore-Materials.md`
- Screenshot source: `screenshots/store/<locale>`, generated by `script/create_store_screenshots.py`
